...
pub struct SgfExporter<'a> { tree: &'a GameTree }

impl<'a> SgfExporter<'a> {
    pub fn new(tree: &'a GameTree) -> Self { Self { tree } }
    ...
    fn write_properties(&self, node: &crate::sgf::tree::Node, f: &mut String) -> std::fmt::Result {
        let order = [Property::GM, Property::FF, Property::SZ, Property::KM, Property::HA,
            Property::PB, Property::PW, Property::RE, Property::DT, Property::RU,
            Property::B, Property::W, Property::AB, Property::AW, Property::AE, Property::C];
        for prop in &order {
            if let Some(vals) = node.get(prop.clone()) { self.write_prop(f, &prop.to_string(), vals)?; }
        }
        for (prop, vals) in &node.data {
            if !order.iter().any(|p| p == prop) { self.write_prop(f, &prop.to_string(), vals)?; }
        }
        Ok(())
    }
    
    fn write_prop(&self, f: &mut String, name: &str, values: &[String]) -> std::fmt::Result {
        for v in values { write!(f, "{}[{}]", name, Self::escape(v))?; }
        Ok(())
    }
